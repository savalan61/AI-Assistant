...
class LoginThrottle:
    ...
    def __init__(self, max_failures: int, window_seconds: int) -> None:
        ...
        # {(scope, value): [failure timestamps, oldest first]}; never exported.
    ...
    def check(self, client_ip: str, login: str, now: datetime) -> None:
        ...
        database work. Naive ``now`` values are rejected rather than guessed as
        local time (the established project convention).
        """
