...
* **Two keys.** Failures are counted per client IP *and* per submitted login,
  ...
  one login goes unnoticed. There is one broker, so the login alone is the
  complete identity to count against.
...
attacker who floods one login can temporarily lock that customer out for the
configured window. The window is intentionally short and configurable, so the
blast radius stays one account for one window.
"""
import threading
...
# Bucket key: (scope, subject). The IP bucket is ("ip", <address>) and the
# login bucket is ("login", <submitted login>). Keeping the parts separate —
# instead of joining them with a delimiter — means no submitted value can be
# shaped to collide with the other bucket.
_BucketKey = tuple[str, str]


def _normalize(value: str) -> str:
    """Case- and padding-insensitive key part."""
...
class LoginThrottle:
    ...
    def check(self, client_ip: str, login: str, now: datetime) -> None:
        ...
        ``login`` is the value as submitted, normalized here so casing or padding
        cannot move a failure into a different bucket. Called before any
        credential lookup, so a throttled attempt costs no database work. Naive
        ``now`` values are rejected rather than guessed as local time (the
    ...
    def record_success(self, client_ip: str, login: str) -> None:
        """Clear this client's and this login's counters after a valid login."""
        with self._lock:
            for key in self._keys(client_ip, login):
    ...
    @staticmethod
    def _keys(client_ip: str, login: str) -> tuple[_BucketKey, ...]:
        # The login is normalized so casing or padding cannot evade the counter.
        # The IP key stays login-independent: one address is one address.
        return (("ip", client_ip), ("login", _normalize(login)))
