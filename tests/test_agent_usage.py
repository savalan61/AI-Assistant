...
def test_below_quota_is_admitted() -> None:
    ...
    limiter.check_and_consume(100, DAY_ONE)
    limiter.check_and_consume(100, DAY_ONE)
...
def test_quota_resets_on_the_next_utc_day() -> None:
    ...
    limiter.check_and_consume(100, DAY_TWO)
...
def test_users_have_independent_quotas() -> None:
    ...
    # A different customer is unaffected: the quota is per user.
    limiter.check_and_consume(200, DAY_ONE)
...
def test_non_utc_time_is_normalized_to_its_utc_day() -> None:
    ...
    limiter.check_and_consume(100, DAY_ONE)
    with pytest.raises(UsageLimitExceededError):
        limiter.check_and_consume(100, ahead)
...
def test_naive_now_is_rejected_loudly() -> None:
    ...
    with pytest.raises(ValueError, match="timezone-aware"):
        limiter.check_and_consume(100, DAY_ONE.replace(tzinfo=None))
